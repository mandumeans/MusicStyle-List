...
public class MusicStyleListMediaPlayerController extends RelativeLayout {
	...
	public void settingController(int type){
		if(type == Define.ONLY_YOUTUBE_TYPE){
	...
	//Setting Progress
	public void setMediaPlayerInfo(boolean enable){
	...
	private void initControllerView(View v) {
		...
		mTopLayout = (LinearLayout) v.findViewById(R.id.linearlayout_top);

		DisplayMetrics metrics = mContext.getResources().getDisplayMetrics();
		int bottomLayoutHeight = metrics.widthPixels / 5;

		mBottomLayout = (LinearLayout)v.findViewById(R.id.linearlayout_bottom);
		LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT
				, bottomLayoutHeight);
		mBottomLayout.setLayoutParams(param);
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if(mPlayer != null){
				if(mPlayer.isMediaPlayerServiceRunning()){
					int pos;
					pos = setProgress();
	...
	public void runControllerHandler(){
		mControllerHandler.sendEmptyMessage(Define.HANDLER_WHAT);
	}
	...
	public interface MediaPlayerControl {
		...
		void    seekTo(int pos);
		boolean isPlaying();
		boolean isMediaPlayerServiceRunning();
	}
}
