...
public class ExtraDao {
	...
	public ArrayList<BaseModel> getTrackData() {
		...
		/*
		 select _id, _title, _thumbnail, _upload_date, duration, _viewCount, _trackType
		 from tb_user_play_list
		 */
		
		String sql = "select " + UserPlayListDao.ID + ", " + UserPlayListDao.TITLE + ", " + UserPlayListDao.THUMBNAIL +
					", " + UserPlayListDao.UPLOAD_DATE + ", " + UserPlayListDao.DURATION +
					", " + UserPlayListDao.VIEW_COUNT + ", " + UserPlayListDao.TRACK_TYPE + ", " + UserPlayListDao.NO +
		...
		try {
			...
			if (cursor.moveToFirst()) 
			{
				do {
					...
					model.uploaded = cursor.getString(3);
					model.duration = cursor.getInt(4);
					model.viewCount = cursor.getInt(5);
					model.trackType = cursor.getInt(6);
					model.no = cursor.getInt(7);
